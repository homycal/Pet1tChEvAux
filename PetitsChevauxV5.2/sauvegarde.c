...
void ChargerSauvegarde(int whitelist[], Pion jeu[], int *joueurDuTour, int *scoreDes){
    FILE *fichier = NULL;
    ...
    if(fichier != NULL){ //Si le fichier est bien chargé en mémoire
      ...
      if(fscanf(fichier, "%d;%d;%d;%d",&whitelist[0], &whitelist[1], &whitelist[2], &whitelist[3])!=4){ //fscanf renvoie le nombre de paramètres lus
      ...
      for(int i=0; i<16; i++){
        if(fscanf(fichier,"%d;%d;%d\n",&jeu[i].plateau, &jeu[i].numCase, &jeu[i].decompteCase)==3){
          jeu[i].numPion=i;
...
void EcrireSauvegarde(int whitelist[], Pion jeu[], int joueurDuTour, int scoreDes ){
  FILE *fichier = NULL;
