...
int DetectionBlocage(Pion jeu[], int pionDuTour, int scoreDes){
  ...
  for(int i=0; i<16; i++){
    for(int j=0;j<scoreDes;j++){
      if(jeu[i].couleurPion != jeu[pionDuTour].couleurPion && (jeu[i].numCase>jeu[pionDuTour].numCase && (jeu[i].numCase)<=(jeu[pionDuTour].numCase+scoreDes-1)%56) && jeu[i].plateau==EXTERIEUR && jeu[pionDuTour].plateau==EXTERIEUR){
