...
//Fonction testant la condition de fin
...
int Mouvements(int joueurDuTour, int scoreDes, Pion jeu[], int *action, int *pionDuTour, int whitelist[], int joueurIA[]){
  ...
  if(jouable){
    do{
      ...
      if(joueurIA[joueurDuTour]==-1){ // je test cela au lieu du mode de jeu afin de pouvoir ajouter des IA au multijoueur
        *pionDuTour = LireEntier(whitelist, jeu ,1, joueurDuTour, scoreDes, joueurIA);//si ce n'est pas une IA qui joue
      }else{
        *pionDuTour = ChoixPionIA(valide, jeu, scoreDes);

      }

  }while(*pionDuTour!=valide[0] && *pionDuTour!=valide[1] && *pionDuTour!=valide[2] && *pionDuTour!=valide[3]);
  printf("Le pion %d à été choisi !\n", *pionDuTour);
  sleep(2);
...
int DeplacementPion(int joueurDuTour, int scoreDes, int action, int pionDuTour,const char couleur[][6], Pion jeu[]){//Fonction qui se charge de déplacer le pion
  switch(action){
    case 1:// On sort le pion de l'écurie
...
void TestDeplacement(Pion jeu[], int joueurDuTour, int scoreDes, int valide[]){
  for(int i=0; i<4; i++){
    ...
    int caseTest, piedCol=(((joueurDuTour*14)-1)+56)%56;
    ...
    else if(block!=-1 && jeu[pionTest].decompteCase!=0){
      printf("le pion %d est bloqué par le pion %d\n", pionTest, block);
    }
    //Si le pion est bloqué car il dépasse ça case du CENTRE
    else if(jeu[pionTest].plateau==EXTERIEUR && jeu[pionTest].decompteCase-scoreDes<=-1 && jeu[pionTest].numCase != piedCol){
    ...
    else if(jeu[pionTest].decompteCase==0 && block2!=-1){ // ne fonctionne que si le pion se trouve au pied de sa colonne
      printf("Le pion %d est bloqué par le pion %d\n", pionTest, block2);
    }
